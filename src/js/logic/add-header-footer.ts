...
export async function addHeaderFooter() {
  showLoader(String(t('alerts.addingHeaderFooter')));
  try {
    ...
    if (indicesToProcess.length === 0) {
      throw new Error(
        String(t('alerts.invalidPageRangeSpecified'))
      );
    }
    ...
    console.error(e);
    showAlert(String(t('alerts.error')), e.message || String(t('alerts.couldNotAddHeaderFooter')));
  } finally {
    hideLoader();
  }
}
